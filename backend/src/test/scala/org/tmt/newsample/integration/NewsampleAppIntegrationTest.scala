package org.tmt.newsample.integration
...
import org.tmt.newsample.NewsampleWiring
import org.tmt.newsample.core.models.{AdminGreetResponse, GreetResponse, UserInfo}
import org.tmt.newsample.http.HttpCodecs

import scala.concurrent.duration.DurationInt
import scala.concurrent.{Await, ExecutionContext}

class NewsampleAppIntegrationTest extends ScalaTestFrameworkTestKit with AnyWordSpecLike with Matchers with HttpCodecs {
  ...
  val newsampleAppPort                    = 8085
  val newsampleWiring                     = new NewsampleWiring(Some(newsampleAppPort))
  val appConnection: HttpConnection    = newsampleWiring.settings.httpConnection
  ...
  protected override def beforeAll(): Unit = {
    ...
    newsampleWiring.start(Metadata.empty).futureValue
  ...
  protected override def afterAll(): Unit = {
    ...
    newsampleWiring.stop().futureValue
    super.afterAll()
  }

  "NewsampleWiring" must {

    "start the newsample app and register with location service" in {
